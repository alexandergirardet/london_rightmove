...
def plot_geo_data(gdf, column):
    ...
    gdf.plot(column=column, cmap='viridis', legend=True, ax=ax,
             legend_kwds={'label': f"{reversed_options[column]}", 'orientation': "horizontal"})
...
def calculate_amenity_walk_score(distances, amenity_weights):
    ...
    for amenity, distance in distances.items():
        ...
        weights = amenity_weights.get(amenity, [1])  # Default weight if amenity not found
...
st.write('Adjust the sliders below to simulate distances to different amenities and calculate a simplified walk score.')
