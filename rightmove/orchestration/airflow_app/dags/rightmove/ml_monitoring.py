...
def fetch_reference_df():
    reference_data = pd.read_csv("gs://rightmove-artifacts-ml/data/2024-02-17-14-18-14/train.csv", index_col=0)
...
def load_data_from_mongo(collection_name, fields, timestamp_field):
    ...
    two_hours_ago = datetime.now() - timedelta(hours=72)
...
def fetch_latest_batch():
    property_fields = {"id": 1, "bedrooms": 1, "bathrooms": 1, "location": 1, "price": 1, "listingUpdate": 1,
                       "firstVisibleDate": 1}
    property_df = load_data_from_mongo("properties", property_fields, "extraction_timestamp")

    walk_score_fields = {"id": 1, "scores": 1}
    walk_score_df = load_data_from_mongo("walk_scores", walk_score_fields, "processing_timestamp")
...
def load_predictions_from_gcs(folder_name):
    current_data = pd.read_csv(f"gs://rightmove-artifacts-ml/predictions/{folder_name}/current.csv", index_col=0)
    reference_data = pd.read_csv(f"gs://rightmove-artifacts-ml/predictions/{folder_name}/reference.csv", index_col=0)

    return current_data, reference_data


def monitor_datasets(current_data, reference_data, **kwargs):
    ...
    performance_data = metric_extractor.get_performance_metrics(current_data, reference_data)

    prediction_data = metric_extractor.get_target_drift_metrics(current_data, reference_data)

    drift_data = metric_extractor.get_data_drift_metrics(current_data, reference_data)

    quality_data = metric_extractor.get_data_quality_metrics(current_data, reference_data)
    ...
    metric_extractor.load_metrics_to_postgres(prediction_data, 'prediction_drift', loading_timestamp=fake_timestamp)
    metric_extractor.load_metrics_to_postgres(performance_data, 'performance', loading_timestamp=fake_timestamp)
    metric_extractor.load_metrics_to_postgres(drift_data, 'drift', loading_timestamp=fake_timestamp)
    metric_extractor.load_metrics_to_postgres(quality_data, 'quality', loading_timestamp=fake_timestamp)
...
def generate_predictions(current_data=None, reference_data=None):
    ...
    new_logged_model = 'runs:/5c5b195cf1b74219993b436489545b7a/random-forest'
...
load_predictions_to_gcs_task = PythonOperator(
    task_id='load_predictions_to_gcs',
    python_callable=load_predictions_to_gcs,
    dag=dag
)
